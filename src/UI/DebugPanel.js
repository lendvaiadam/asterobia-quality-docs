...
export class DebugPanel {
    ...
    setupProfiler() {
        ...
        // === DEBUG OVERLAYS FOLDER ===
        const debugFolder = folder.addFolder({ title: 'Debug Overlays', expanded: false });
        
        const debugToggles = {
            waypointOverlay: true,
            pathPlannerDots: false
        };

        // WAYPOINT DEBUG OVERLAY
        debugFolder.addBinding(debugToggles, 'waypointOverlay', { label: 'Waypoint Debug' }).on('change', (ev) => {
            if (this.game.debugOverlay) {
                if (ev.value) {
                    this.game.debugOverlay.show();
                } else {
                    this.game.debugOverlay.hide();
                }
            }
        });

        // PATH PLANNER DEBUG (Shortest Path Dots)
        debugFolder.addBinding(debugToggles, 'pathPlannerDots', { label: 'Path Debug Dots' }).on('change', (ev) => {
            if (this.game.pathPlanner) {
                this.game.pathPlanner.setDebugEnabled(ev.value);
                // Clear existing dots if disabled
                if (!ev.value) {
                    this.game.pathPlanner.clearDebug();
                }
            }
        });
    ...
    setupUnitControls() {
        ...
        // Set initial state
        if (this.game.pathPlanner) {
            this.game.pathPlanner.setDebugEnabled(pathDebugParams.showDots);
        }
    }

 ...
 * MOVE_INPUT processing, and PHASE2A env gating.
 *
...
describe('Phase 2A: SERVER_SNAPSHOT schema', () => {
    ...
    it('createServerSnapshot produces valid message', () => {
        const msg = createServerSnapshot({
            ...
            units: [{ id: 1, ownerSlot: 0, px: 0, py: 0, pz: 0, heading: 0, speed: 0, hp: 100 }]
        });
...
describe('Phase 2A: HeadlessUnit.applyInput', () => {
    ...
    it('diagonal normalization: W+A speed equals W-only speed', () => {
        ...
        const cardinalSpeed = Math.sqrt(
            cardinalUnit.velocity.x ** 2 + cardinalUnit.velocity.z ** 2
        );
        const diagonalSpeed = Math.sqrt(
            diagonalUnit.velocity.x ** 2 + diagonalUnit.velocity.z ** 2
        );
    ...
    it('ignores non-MOVE_INPUT commands', () => {
        ...
        expect(unit.velocity.x).toBe(99); // unchanged
    });
...
describe('Phase 2A: Room tick loop', () => {
    ...
    beforeEach(() => {
        ...
        room = new Room('test-room', {
            broadcast: (roomId, snapshot) => {
                broadcastLog.push({ roomId, snapshot });
            }
        });
    ...
    it('snapshot contains unit with correct fields', () => {
        tickRoom(room, 1);
    ...
    it('MOVE_INPUT advances unit position after tick', () => {
        const startX = room.units[0].position.x;
        const startZ = room.units[0].position.z;

        room.receiveInput(0, { type: 'MOVE_INPUT', forward: true, backward: false, left: false, right: false });
        tickRoom(room, 1);

        // Forward = -Z direction
        expect(room.units[0].position.z).toBeLessThan(startZ);
        expect(room.units[0].position.x).toBe(startX);
    });
    ...
    it('routes MOVE_INPUT to correct unit by sourceSlot', () => {
        ...
        // Unit 1 (slot 1) moved
        expect(room.units[1].position.z).toBeLessThan(unit1Start.z);
    });
...
// ========================================
// 4. Room: createUnitForPlayer
// ========================================

describe('Phase 2A: Room.createUnitForPlayer', () => {
    it('creates unit with deterministic start position based on slot', () => {
        resetEntityIdCounter();
        ...
        expect(u0.position.x).not.toBe(u1.position.x);
